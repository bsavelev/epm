...
//for Debian RPM skip perl req:
  fputs("%undefine __perl_requires\n", fp);
  fprintf(fp, "Name: %s\n", prodname);
  ...
  */
  ...
  if (!strcmp(platform->machine, "intel"))
  {
    if (run_command(NULL, EPM_RPMBUILD " %s %s -bb "
                    EPM_RPMARCH "i386 %s%s", def1, def2,
  ...
  {
    if (run_command(NULL, EPM_RPMBUILD " %s %s -bb "
                    EPM_RPMARCH "powerpc %s%s", def1, def2,
                    build_option, specname))
      return (1);
  }
  else if (run_command(NULL, EPM_RPMBUILD " %s %s -bb "
                       EPM_RPMARCH "%s %s%s", def1, def2, platform->machine,
                       build_option, specname))
    return (1);
